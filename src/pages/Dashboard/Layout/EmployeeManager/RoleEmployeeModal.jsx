...
import { updatePermission } from "~/services/employee";
...
export default function RoleEmployeeModal({ employee, onClose, onSave }) {
  ...
  useEffect(() => {
    ...
    getPermission(employee._id)
      .then((res) => {
        setChecked(res.data.permissions || []); // tick quyền đã có
        setIdPermission(res.data._id);
      })
  ...
  const handleSave = () => {
    updatePermission(idpermission, { permissions: checked });
    onClose();
  };
